...
import ist.utl.pt.bbcm.server.enums.DIRECTION;
...
public class Matrix {
	...
	public static boolean posIsFree(int[] pos) {
		...
		try{
			...
			System.out.println("Array index is out of bounds.x:"+x+".y:"+y);
		}
		return true;
	}
	...
	public static String updateRobotsPosition() {
			...
			for(int row=0; row < mapMatrix.length; row++){
				...
				if(thisChar=='R'){
					...
					if(posIsFree(nextPos)){
						msgToSendToPlayers += row+","+col+","+dir.x+","+dir.y+";";
						inicio.add(currPos);
						destino.add(nextPos);
					}
				}
		...
		while (numAcks<1) {
                Socket clientSocket;
				try {
					clientSocket = Main.serverSocket.accept();
